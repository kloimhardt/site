...
<td style="vertical-align: top">
<div id="blocklyDiv14" style="height: 600px; width: 800px"></div>
<script>
...
<script>
r19 = () => brender(" (call (log2b 220 7) 440)", "blocklyDiv19");
var ws19 = r19();
...
Not only can log2b be plotted. -- It can also be called with a number. The number 440 is the frequency of the standard pitch A. -- What might be surprising is that log2b can also be called with a symbol. I chose x0 as the symbol in this example. But it is arbitrary. It could also have been x1. Remember that log2 does not know anything about x or y or z. --  Also, the step argument does not need to be an integer, it can well be 0 point 7.
</div>
</details>
...
In my pictorial representation, I'd like to do the same. In practice, this does not mean that the D is replace by the pipe, but rather the call procedure.
</div>
</details>
...
This means that also the call to log2b is colloquially done with a pipe. -- Actually, the implementation of pipe is done quite easily with a few lines of Python.
</div>
</details>
