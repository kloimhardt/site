...
While it is the log2 that fits the notes, it is not the plain log2. There need to be some scale factors. -- If you want, you can play with the formla yourself, because this notebook is online at the following address
...
I can plot log2 because it is a methematical object. -B- But still I cannot plot the procedure Poe. -C- I need an operation called "partial" to transform Poe into the proper target to plot. -D- The procedure named "partial" takes "Poe" as an argument. -- Then I can perform the plotting. -E-  Here is the main theme in spades: "partial" taking a procedure to deliver the target to plot. -F- I turn all this into yet another procedure named log2b -- That is very conveneint for plotting. -G-
...
Not only can log2b be plotted. -B- Log2b can also be called. -C- Not only can log2b be called with data of type number. Log2b can also be called with a symbol. -DE- Using symbols along with numbers is possible thanks to a package named Sympy. Here you see that for the name of the symbol I chose x-naught. But x-naught is only the default of possible choices. As a symbol, I can also choose x1. -F- This freedom of choice is because for log2b, the x1 is just some data that happens to be of type symbol. -G- Before being able to process some data, log2b needs to be initialized with two parameters. But those do not need to be of type integer. The float O point 7 does nicely as well. -a-
</div>
</details>
...
If I initialize log2b with O point seven and calculate its derivative, as a result I almost get one over x. To become exact, I need to switch to "ln", the natural logarithm. The pictures above can, within this presentation, be called its pinnacle. As a kind of confirmation, I'd like to drop some names. The procedure named "call" would in math be recognised as a functional and D as an operator. In computer science, both is called functions of higher order. The natural logarithm "ln" of course is a function.
</div>
</details>
...
Inspired by math, I use pipe in my pictures. -B- Note that it is not the D operator that is replaced by the pipe, but it is the "call" that is dropped. -- Not only can "pipe" be used in pictorial representations. The pipe can also be used within Python code. -C- You might think that my pipe notation is hard to implement. -- This is not so. My custom pipe is just a few lines of Python.
</div>
</details>
...
The pipe symbol is an invented alternative notation. -B- I discoverd that this invention is quite handy for explaining the separation of computation and data. -C- In my dataset the highest frequency is 8000 cycles per second. -D-  I'd like to cut that to 900. -E- So I apply a filter -- and plot that. -F- I want to move the filtering to the left in front of the first pipe. For this, I use a function composition. -G- The main advantage of this composition is that it is completely decoupled from input sources and output format. -- What I showed here as moving stuff to the left is commonly known as the method of "transducers" -ab- For sure you can insert differentiation into this composition as easily as ever. -c-
</div>
</details>
