...
To make it fit the notes, you need to insert some scale factors into the logarithmic formula. -- You can play with the formula youself. This notebook is online at the following address

<a href="https://kloimhardt.github.io/site/log.html" style="font-size: 24px">https://kloimhardt.github.io/site/log.html</a>

But I have jumped ahead. I need to start with plotting the plain log2.
</div>
</details>
...
And here, I hit the main message of my presentation. It is this. -- It is the log2 that you plot. This is so obvious that I'd like to talk about what log2 is not. It is not the log2 of x or y or z. It is not some string or text that contains the word log2. So what is log2? It is a mathematical object. I restrict myself to plotting those things only and show a way to overcome this restriction. Mastering this challange will lead to fruitful insights beyond plotting, namely for calculating derivatives.
</div>
...
<span onClick="abws5('(Poe intersect steps x)'); abws5('(Poe intersect steps 440)'); abws5('(def :tiles/slot (return :tiles/slot))')" style="color:blue">C</span>

<div class="desc" id="blocklyDiv5_t">
To plot the notes, I come back to the formula I showed at the beginning. This formula is the result of my trials and errors, a process I do not want to talk about. I just present it as the result of my plots and mistakes. The formula is the digested essence. Within my restrictions, you cannot plot this formula directly -B- That's not something you can plot. This is the main theme again. I need a proper mathematical target to plot. -C- To get such a target, I need to wrap this formula into a procedure. -- The procedure itself I gave the name Poe. It takes the arguments intersect, steps and the frequency x. -- The procedure returns the formula. -- You have to replace the numbers by the chosen names. -D- I provide the procedure as Python code.
</div>
</details>
...
The procedure named Poe is not yet the mathematical target. The procedure Poe which does the calculation according to the original formula shares the same fate as this formula: also Poe cannot be plotted directly. -A- To make the desired mathematical target out of Poe, I need to do an operation that is a standard in computer science. You call a procedure named "partial" and give it "Poe" as the argument. Here is the main theme in spades: "partial" taking a procedure to deliver the target to plot. -B- All this can in turn be wrapped in yet another procedure. -C-
</div>
</td>
</tr></table>
...
Not only can log2b be plotted. Log2b can also be called. -B- What might be surprising is that log2b can be called with a symbol. I chose x0 as the symbol in this example. -x1- But it was arbitrary. It can also choose x1 as a symbol. You know, log2 does not know anything about x or y or z. -0.7-  Also, the step argument does not need to be an integer, it can well be 0 point 7.
</div>
</details>
...
In fact, the case 0.7 is an interesting one. -B- As any curve, it can be approximated by triangles. -C- I move the triangles to the ground level. -D- Now the curve known as "one over x" fits their hights quite nicely. -E- You can also try the other way around -F- That does not work quite as well. -G- The trick is to shrink the area of the triangle while retaining its shape.
</div>
</details>
...
If I run the Python code with 0.7, I nearly get one over x for the derivative. To get the exact result, I need to switch to the natural logarithm ln. The pictures above are the pinnacle of my presentation. To prove the importance of them, I drop you some names. In math, the procedure named "call" would be akin to a functional, D is an operator. In computer science, both are called higher order functions. ln is of course a function.
</div>
</details>
...
I'd like to wind down with a note on notation. The usual notation for the derivative at point x-naught is by unsing the vertical bar, colloquially called pipe.
</div>
</details>
...
In my pictorial representation, I also like to use the pipe symbol. Note that it is not the D operator that is replaced by the pipe, but it is the call functional that is dropped.
</div>
</details>
