...
It is not the plain log2 that fits the notes. There need to be some scale factors. -- Because this notebook is online at the following address

<a href="https://kloimhardt.github.io/site/log.html" style="font-size: 24px">https://kloimhardt.github.io/site/log.html</a>

You can play with the formula yourself. But I have jumped ahead. The story starts with plotting the plain log2 which already hits the main message of my presentation.
</div>
</details>
...
The message is this. -- It is the log2 that you plot. This is so obvious that I'd like to state what log2 is not. It is not the log2 of x or y or z. It is not some string or text that contains the word log2. So what is log2? It is a mathematical object. The plot routine I am using to plot log2 is a certain Python procedure that is severely restricted. Only mathematical targets, and only those, can be plotted. The tools utilised to work within this restriction will continue to serve other purposes beyond plotting. The pinnacle of this presentation shows those tools providing a new way for the notation and calculation of derivatives.
</div>
...
Because I am aware that some time for digestion is needed, my viewers might be relieved that I now direct this column towards the practical goal. -- I need to get more to something like this. And so, I get back to the formula that appeared at the start.
</div>
...
The formula I showed at the start -B- is not something you can plot. This is the main theme again. I need a proper mathematical target to plot. -C- To get such a target, I need to wrap this formula into a procedure. -- The procedure itself I gave the name Poe. It takes three arguments. Two parameters and the frequency x. -- The procedure does the necessary computation, it returns the formula-computed value. -- In its code, you have to replace the numbers with the two parameters. -D- I provide the procedure as Python code.
</div>
</details>
...
The procedure named Poe is not yet the mathematical target. This procedure doing the calculation shares the same fate as its formula: also Poe cannot be plotted directly. -A- To get the mathematical target desired for plotting, I need to perform an operation named "partial". -- You call a procedure named "partial" and give it "Poe" as the argument. Here is the main theme in spades: "partial" taking a procedure to deliver the target to plot. -B- All this can in turn be wrapped in yet another procedure. -CA_-
</div>
</td>
</tr></table>
...
::: {.column-margin}
<a href="#6" onClick="var ws6t=brender('(def (log2b intersect steps) (return (partial Poe intersect steps)))', 'blocklyDiv14'); addBlocks(ws6t)('(plot (log2b 220 7) [1 2 4 8])'); cleandiv('blocklyDiv14_t')">_</a>
<details>
<summary>A</summary>
<div style="width: 300px">
```{python}
...
Not only can log2b be plotted. Log2b can also be called. -B- That log2b can be called with a symbol might be a surprise. That is thanks to a package named Sympy. Here you see that I chose x-naught as the symbol. -CD- But x-naught is only the default of possible choices. I can also choose x1 as a symbol. You know, log2b does not know a thing about x (or y or z). -EF- Also none of the parameters need to be a whole number. O point 7 does nicely as well.
</div>
</details>
...
The case of O point seven is a contrived invention of mine. I would not bother you with it if I had not discovered it to be a segue to the natural logarithm. -B- In general, you can fit a series of triangles onto any curve. -C- Moving those triangles to the ground level reveals -D- that a new curve "one over x" fits their heights quite nicely. -E- You can also try the other way around -F- That does not work quite as well. -G- The trick is to shrink the area while retaining the angles.
</div>
</details>
...
Curves and triangles that connect them to other curves. This smells of derivative. -- Differentiating log2b is simple and easy.
</div>
</details>
