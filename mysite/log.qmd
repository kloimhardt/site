...
I can plot log2 because it is a methematical object. -B- But I cannot plot the procedure Poe -C- I need an operation called "partial". -D- You call a procedure named "partial" and give it "Poe" as the argument. Then I can perform the plotting. -E-  Here is the main theme in spades: "partial" taking a procedure to deliver the target to plot. -F- I turn all this into yet another procedure named log2b -- That is very conveneint for plotting. -G-
</div>
</td>
</tr></table>
...
Not only can log2b be plotted. Log2b can also be called. -B- The twist here is that I called log2b not with a number but with a symbol. Using symbols along with numbers is possible thanks to a package named Sympy. Here you see that for the name of the symbol I chose x-naught. But x-naught is only the default of possible choices. I can also choose x1 as a symbol. -C-D- This freedom of choice is because log2b does not know a thing about x (or y or z). -E-DF- Also none of the parameters need to be a whole number. O point 7 does nicely as well.
</div>
</details>
...
Inspired by math, I use pipe in my pictures. Note that it is not the D operator that is replaced by the pipe, but it is the call that is dropped.
</div>
</details>
...
The goal of this presentation was to show some merits of my way of plotting. This goal has been reached with the "partial" procedure to produce functions of higher order. The pipe symbol has just been an invention for notation. But I discoverd that the pipe symbol is quite handy for pointing at an idea to speed up the calculation. -BC- To start, I also for plotting introduce the pipe symbol. -D- You might think that my pipe notation is hard to implement. This is not so. My custom pipe is just a few lines of Python.
</div>
</details>
