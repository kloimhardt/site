...
It is the log2 that fits the notes, but it is not the plain one. There need to be some scale factors. -- If you want, you can play with the formla yourself, because this notebook is online at the following address

<a href="https://kloimhardt.github.io/site/log.html" style="font-size: 24px">https://kloimhardt.github.io/site/log.html</a>

But I have jumped ahead. The story starts with the language Python and plotting just the plain log2. And that already hits upon the main message of my presentation.
</div>
</details>
...
I can plot log2 because it is a methematical object. -B- But I cannot plot the procedure Poe -C- I need an operation called "partial" to transform Poe into the proper target to plot. -D- You call a procedure named "partial" and give it "Poe" as the argument. -- Then I can perform the plotting. -E-  Here is the main theme in spades: "partial" taking a procedure to deliver the target to plot. -F- I turn all this into yet another procedure named log2b -- That is very conveneint for plotting. -G-
...
<span onClick="abws19('x1');abws19('0.7')" style="color:blue">F</span>
<div class="desc" id="blocklyDiv19_t">
Not only can log2b be plotted. -B- Log2b can also be called. -C- Not only can log2b be called with a number but also with a symbol. -DE- Using symbols along with numbers is possible thanks to a package named Sympy. Here you see that for the name of the symbol I chose x-naught. But x-naught is only the default of possible choices. I can also choose x1 as a symbol. -F- This freedom of choice is because log2b does not know a thing about x (or y or z). -- Also none of the parameters need to be a whole number. O point 7 does nicely as well. -G-
</div>
</details>
...
Triangles and curves like above bring to mind the concept of derivative.  -D- Taking the derivative of log2b is simple and easy.
</div>
</details>
...
Inspired by math, I use pipe in my pictures. -B- Note that it is not the D operator that is replaced by the pipe, but it is the call that is dropped. -- The pipe can also be implemented in Python. -C- You might think that my pipe notation is hard to implement. -- This is not so. My custom pipe is just a few lines of Python.
</div>
</details>
...
<div id="blocklyDivpipe2" style="height:300px"></div>
<script>
var r16 = () => brenderT("(plot ln [1 2 4 8])", "blocklyDivpipe2", true);
ws11p = r16();
</script>
<span style="color:blue">A</span>
<span onClick="addBlocks(ws11p)('((plotting :tiles/slot) :tiles/slot)')" style="color:blue">B</span>
<div class="desc" id="blocklyDivpipe2_t">
The goal of this presentation was to show some merits of my way of plotting. This goal has been reached with the "partial" procedure to produce functions of higher order. The pipe symbol has just been an invention for notation. But I discoverd that the pipe symbol is quite handy for pointing at an idea to speed up the calculation. -BC- To start, I also for plotting introduce the pipe symbol.
</div>
</details>

::: {.column-margin}
<a href="#16" onClick='brenderT("((plotting ln) [1 2 4 8])", "blocklyDivpipe2", true)'>_</a>
...
<script>
wsTra = brenderT("((plotting ln) frequencies)", "blocklyDivTrans", true);
var abwsTra = addBlocks(wsTra);
</script>

<span style="color:blue">A</span>
<span onClick="abwsTra('((filtering (smaller 880)) :tiles/slot)')" style="color:blue">C</span>
<span onClick="abwsTra('(compose (mapping :tiles/slot) :tiles/slot)')" style="color:blue">E</span>
<span onClick="abwsTra('(D :tiles/slot)')" style="color:blue">G</span>
<div class="desc" id="blocklyDivTrans_t">
In a way, the pipe explicitely depicts the processing time. When plotting points, the pipe depicts the time spent on calculating their place. -B- To plot all notes, I need to load the frequencies from external data. The data contains the frequencies up to 8000 cycles per second. -C- I'd like to only focus on the frequencies below 880 Hz, so I apply the respective filter to the data -- and plot that. -D- In priciple I am done. But I'd like to have the filtering not in the middel position, but in front of the first pipe. -E- The trick is to use compose which you see in the green tile. -F- As soon as you compose things in front of the pipe like this, you open up a way to speed up the calculation. This is because the concept commonly known as "transducers" in principle becomes applicable. I have to admit though that I was not able to find a Python transducer library that outperforms the standard. -G- For sure you can insert differentiation into this composition as easily as ever. -a-
