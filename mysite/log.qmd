...
> Pythagoras has shown that the pitch of a tone depends on the length of a cord. Intervals are produced by simple numerical ratios of this length. This was the first successful reduction of quality to quantity and therefore the beginning of Science.  
...
(add-blocks '(def (log2a intersect steps x) :tiles/slot)))
...
function mutationCallback(customFn, mutationsList, observer) {
      mutationsList.forEach((mutation) => {
        ...
        console.log(customFn(mutation.target.childNodes[0].innerText));
